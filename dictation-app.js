(() => {
  ...
  root.innerHTML = `
    <div class="wrap">
      ...
      <main class="grid"><section class="card quiz"><div class="counter" id="count">词库加载中</div><div class="prompt" id="prompt">loading</div><div class="hint" id="hint">准备中</div><div id="choiceArea" class="choices"></div><button id="unknownBtn" class="unknownBtn">不认识</button><div id="answerArea" class="answerRow"><input id="answer" type="text" inputmode="latin" autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false" placeholder="输入英文拼写"><button class="primary" id="submitBtn">确认</button></div><div class="feedback" id="feedback">正在准备题目</div><div class="nav"><button id="focusBtn">回到输入框</button><button id="nextBtn2">下一题</button></div></section><aside><section class="card"><h2>统计</h2><div class="stats"><div class="stat"><span>正确</span><b id="right">0</b></div><div class="stat"><span>错误</span><b id="wrong">0</b></div><div class="stat"><span>未答</span><b id="left">0</b></div></div></section><section class="card" style="margin-top:16px"><h2>题目地图</h2><div class="dots" id="dots"></div></section></aside></main>
  ...
  function setNavActive(active){
    ['prevBtn','nextBtn','skipBtn','finishBtn','nextBtn2'].forEach(id=>{ const el=$(id); if(el){ el.style.display=active?'':'none'; el.disabled=!active; } });
  }
  function renderStartScreen(){
    ...
    $('hint').textContent=mode === 'dictation' ? '开始后会显示中文释义、首字母和词性，请输入完整英文单词。' : '开始后会显示英文单词，请从四个中文选项中选择正确意思。';
  ...
  function normalize(s){ return String(s).normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim().replace(/[’‘`´]/g,"'").replace(/[^a-z0-9]/g,''); }
  ...
  function move(step){ if(!quiz.length) return startQuiz(); if(mode==='dictation'&&step>0){ const r=currentRecord(),typed=$('answer').value.trim(); if(r&&!r.done&&typed){ submitAnswer(); return; } if(r&&!r.done&&!typed){ pop('请先输入答案，或点击“跳过”'); focusAnswer(); return; } } saveTyping(); idx=Math.max(0,Math.min(quiz.length-1,idx+step)); showCard(); focusAnswer(); }
  ...
  async function loadData(){ try{ const urls = Array.isArray(cfg.dataUrls) ? cfg.dataUrls : [cfg.dataUrl]; const texts = await Promise.all(urls.map(async url => { const r=await fetch(url,{cache:'no-store'}); if(!r.ok) throw new Error(r.status); return r.text(); })); bank=parseData(texts.join(nl())).map(item => Object.assign(item, { pos: inferPos(item) })); bank.forEach(item => { item.profile = inferProfile(item); item.displayPos = normalizePos(item.sourcePos) || item.profile || item.pos; }); $('total').textContent=bank.length+'词'; $('start').disabled=false; $('size').querySelector('option[value="all"]').textContent='全部 '+bank.length+'题'; renderStartScreen(); pop('词库已加载'); } catch(e){ setNavActive(false); $('state').textContent='词库加载失败'; $('prompt').textContent='词库加载失败'; $('hint').textContent='请刷新页面重试'; } }
